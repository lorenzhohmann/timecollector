...
router.get('/:id/:from/:to', (req, res) => {
	db.query(
		"SELECT t.id, DATE_FORMAT(date, '%d.%m.%Y') date, TIME_FORMAT(worked_from, '%k:%i') worked_from, TIME_FORMAT(worked_to, '%k:%i') worked_to, TIME_FORMAT(break_from, '%k:%i') break_from, TIME_FORMAT(break_to, '%k:%i') break_to FROM users u JOIN times t ON u.id = t.user_id WHERE u.id = " +
			req.params.id +
			" AND date >= '" +
			req.params.from +
			"' AND date <= '" +
